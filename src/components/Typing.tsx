...
function Typing() {
  ...
  useEffect(() => {
    ...
    const array = [
      currInterval - currTime,
      wordsPerMinute(currInterval - currTime),
    ];
  ...
  function handleUserInput(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    let allCurrChar = wordsArrayRef.current[currWordIndex]
    ...
    if (event.key == 'Backspace') {
      ...
      if (currCharIndex != 0) {
        ...
        const ind = wordsArrayRef.current[currWordIndex - 1]?.childNodes
          .length as number;

        setCurrCharIndex(ind);
        (
          wordsArrayRef.current[currWordIndex - 1]?.childNodes[
            ind - 1
          ] as HTMLSpanElement
        ).classList.add('end-of-char');
      }
      return;
    }
    ...
    if (currCharIndex == allCurrChar.length && event.key == ' ') {
      const lastCharOfCurrentWord = wordsArrayRef.current[currWordIndex]
        ?.childNodes[allCurrChar.length - 1] as HTMLSpanElement;
      lastCharOfCurrentWord?.classList.remove('end-of-char');

      if (
        wordsArrayRef.current[currWordIndex]?.childNodes.length ==
        correctCharsOfWord
      )
        setCorrectWords(correctWords + 1);
      setCorrectCharsOfWord(0);
      setCurrWordIndex(currWordIndex + 1);
      let newTarget = wordsArrayRef.current[currWordIndex + 1]
  ...
  return (
    <div>
      <div
        ...
        className="flex justify-between mb-[50px]"
      >
        <div
          className="ml-[15%] text-4xl cursor-pointer"
          onClick={() => {
            myElementRef.current?.focus();
          }}
        >
      ...
      >
        {wordsArray.map((text, index) => {
          return (
            <div
              key={index}
              className="pr-4 text-2xl font-serif "
              ref={(element) => {
                wordsArrayRef.current[index] = element;
              }}
            >
              {text.split('').map((char) => {
                return <span className="pl-[2px]">{char}</span>;
              })}
            </div>
          );
        })}
      ...
      <div
        className={`${!isOver == true ? 'hidden' : ''}  text-xl text-center mt-[30px]`}
      >
        <Result
          graphData={graphData}
          WordsPerMinute={wordsPerMinute(currInterval)}
      ...
      <input
        ...
        className="text-white hide absolute opacity-0 w-0 h-0"
        ref={myElementRef}
        type="text"
      ></input>
    </div>
